...
async def has_pdf_original(db: AsyncSession, file_id: str) -> bool:
    """Cheap existence check for the file list — whether the cached original
    (see get_original) is specifically a PDF, without reading its bytes off
    disk. Must mirror KnowledgeDetail.svelte's loadExistingOriginalPreview,
    which classifies a fetched original as "pdf" by checking its real
    Content-Type (`contentType.includes("pdf")`) rather than the filename —
    a mismatch here is exactly what made the file-list icon lie (shown for
    non-PDF originals, e.g. a cached .txt/.docx) in the first version of
    this check, which only tested that *some* original file existed.
    """
    row = (
        await db.execute(
            select(OriginalFileBlob.content_type, OriginalFileBlob.stored_path).where(
                OriginalFileBlob.file_id == file_id
            )
        )
    ).first()
    if row is None:
        return False
    content_type, stored_path = row
    return "pdf" in (content_type or "").lower() and Path(stored_path).is_file()
